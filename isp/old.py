...
def home(request):
    try:
        # Fetch customers from the queue
    ...
    try:
        # Disconnect from the connection if needed
        connection.disconnect()
    except Exception as e:
    ...
    try:
        if request.method == "GET":
            ...
            if disabled_customers:
                return Response({"message": f"The following customers have been disabled: {', '.join(disabled_customers)}"}, status=status.HTTP_200_OK)
            else:
